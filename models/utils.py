...
def timegan_trainer(model, fault_number, p, max_seq_len, sup_epochs, emb_epochs, batch_size, device, learning_rate, project_filepath):
    ...
    load_path = f'{project_filepath}/datasets/dataset_fault_{fault_number}.pth'
    ...
    model_path = f'{project_filepath}/GAN Model Config'
    torch.save(model.state_dict(), f"{model_path}/model_fault_{fault_number}.pt")


def timegan_generator(model, fault_number, device, T, max_seq_len, Z_dim, project_filepath):
    """
    ...
    model_path = f'{project_filepath}/GAN Model Config/model_fault_{fault_number}.pt'
    if not os.path.exists(model_path):
        raise ValueError(f"Model for fault {fault_number} not found...")
...
# def create_rolling_windows(data, window_length):
#     """
#     Creates rolling window segments from the input time series data.
    
#     Args:
#     data (np.ndarray): Input time series data of shape (n, m)
#     window_length (int): The number of timesteps in each window, must be <= n
    
#     Returns:
#     np.ndarray: An array of shape (n - window_length + 1, window_length, m) containing rolling windows
#     """
#     # Check if window_length is greater than the number of timesteps
#     if window_length > data.shape[0]:
#         raise ValueError("window_length cannot be greater than the number of timesteps in the data")
    
#     # Initialize an empty list to store the windows
#     n_windows = data.shape[0] - window_length + 1  # Number of windows
