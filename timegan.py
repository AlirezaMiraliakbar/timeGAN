...
from models.utils import create_rolling_windows, is_size_fixed, timegan_trainer, timegan_generator, pad_arrays
from data.utils import save_file, unpad_arrays, segment_time_series
...
class EmbeddingNetwork(torch.nn.Module):
    ...
    def forward(self, X, T):
        ...
        # 128 x 100 x 71 [B x S x H] H: hidden_dim
...
class RecoveryNetwork(torch.nn.Module):
    ...
    """
    def __init__(self, hidden_dim, feature_dim, num_layers, padding_value, max_seq_len):
...
class TimeGAN(torch.nn.Module):
    ...
    def __init__(self, device, feature_dim, Z_dim, hidden_dim, max_seq_len, batch_size, num_layers, padding_value):
        ...
        self.embedder = EmbeddingNetwork(self.feature_dim, self.hidden_dim, self.num_layers, self.padding_value, self.max_seq_len)
        self.generator = GeneratorNetwork(self.Z_dim, self.hidden_dim, self.num_layers, self.padding_value, self.max_seq_len)
        self.discriminator = DiscriminatorNetwork(self.hidden_dim, self.num_layers, self.padding_value, self.max_seq_len)
        self.recovery = RecoveryNetwork(self.hidden_dim, self.feature_dim, self.num_layers, self.padding_value, self.max_seq_len ) 
    ...
    def forward(self, X, T, Z, obj, gamma=1):
        ...
        return loss
# --------------------------------------------- END OF MODELS -----------------------------------------------------------
def dataset_maker(fault_number, p, num_sim, padding_value, project_filepath):
        # Loading training data 
    load_path = f'{project_filepath}/IC and OC data/'
    with open(f'{load_path}OC_train_truncated_0.pkl', 'rb') as file:
            OC_train_truncated_0 = pickle.load(file)
    with open(f'{load_path}OC_train_truncated_40.pkl', 'rb') as file:
            OC_train_truncated_40 = pickle.load(file)
    with open(f'{load_path}OC_train_truncated_80.pkl', 'rb') as file:
            OC_train_truncated_80 = pickle.load(file)
    with open(f'{load_path}OC_train_truncated_120.pkl', 'rb') as file:
            OC_train_truncated_120 = pickle.load(file)

    oc_data_truncated_0 = OC_train_truncated_0[(fault_number-1)*num_sim:(fault_number)*num_sim]
    oc_data_truncated_40 = OC_train_truncated_40[(fault_number-1)*num_sim:(fault_number)*num_sim]
    oc_data_truncated_80 = OC_train_truncated_80[(fault_number-1)*num_sim:(fault_number)*num_sim]
    oc_data_truncated_120 = OC_train_truncated_120[(fault_number-1)*num_sim:(fault_number)*num_sim]

    oc_data_train_all_cases = oc_data_truncated_0 + oc_data_truncated_40 + oc_data_truncated_80 + oc_data_truncated_120

    X = list()
    T = list()

    for i, sim_data in enumerate(oc_data_train_all_cases):
            X.append(sim_data)
            T.append(sim_data.shape[0])
    # X is time series dataset as a PyTorch Tensor of shape [num_samples, seq_length, features]
    
    max_seq_len = max(T) 
    X_padded = pad_arrays(arrays=X, max_seq_len=max_seq_len, padding_value=padding_value)
    ...
    print(f'Timesteps list with inconsistent timesteps has length of {len(T)}! Make sure secure paddding!\n')
    # Making a TorchDataset with X and T (models work with both)
    dataset = TensorDataset(X,T) 
    save_path = f'{project_filepath}/datasets/'
    if not os.path.exists(save_path):
        os.makedirs(save_path)
    torch.save(dataset, f'{save_path}dataset_fault_{fault_number}.pth') 

    return X, T, max_seq_len

def TimeGAN_trainer(device, fault_number, p, max_seq_len, project_filepath):
    ...
    timegan_trainer(model=model, fault_number=fault_number, batch_size=32, device=device, learning_rate=0.0002, project_filepath=project_filepath, p=46, max_seq_len=max_seq_len, sup_epochs=100, emb_epochs=100)
    return model

def model_evaluation(generated_data, original_data):
    '''
    Evaluating the generated timeseries data by comparing with original data

    '''
    retult = np.sqrt(genetated_data - original_data)
...
if __name__ == "__main__":
    ...
    X, T, max_seq_len = dataset_maker(fault_number=1, p=46, num_sim=350, padding_value=100, project_filepath=project_filepath)
    ...
    model_f = TimeGAN_trainer(device=device, fault_number=1, max_seq_len=max_seq_len, p=46, project_filepath=project_filepath)

    #TODO: generate more data with a for loop of this line of code later to be used in the machine learning model training
    for xxx in range(10):
        print("\nGenerating Data...")
        ...
        save_file(file=generated_f1, file_format='pkl', file_name=f'f1_generated_{xxx}', file_path=f'{project_filepath}/IC and OC data/generated data/')
